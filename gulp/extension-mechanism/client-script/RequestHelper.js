...
var RequestHelper = {
    ...
    request: function request(options)
    {
        ...
        return new Promise(function (resolve, reject) {
            ...
            const execute_promise = async function execute_promise() {
                try {
                    const response = await self.doPaginatedRequest(options);
                    resolve(response);
                } catch (error) {
                    reject(error);
                }
    ...
    doPaginatedRequest: async function doPaginatedRequest(options){
        let response = await this.doRequest(options);
        const redirectOptions = this.getRedirectOptions(response);

        if (redirectOptions) {
            response = await this.doPaginatedRequest(redirectOptions);
        }

        return response;
    },
    getRedirectOptions: function getRedirectOptions(response){
        return response.result.redirect_options;
    }
};
